...
if git -C "$REPO_ROOT" ls-files --error-unmatch bun.lock >/dev/null 2>&1; then
  ok "bun.lock is tracked at the service root (Bun will be present in the image)"
...
if [ -n "${BUILD_CMD##*--frozen-lockfile*}" ]; then
  ...
  if (cd "$REPO_ROOT" && bun install --frozen-lockfile --dry-run) >/dev/null 2>&1; then
...
if [ -n "$ENTRY" ]; then
  ...
  if jq -e --arg k "$SCRIPT_KEY" '.scripts[$k] // empty' "$REPO_ROOT/package.json" >/dev/null 2>&1; then
  ...
  cat <<EOF
   ...
   A) BLUEPRINT (recommended - render.yaml stays the source of truth)
      ...
      subcommand, there is no launch/apply. It is a dashboard flow:
        ...
        2. "New +"  ->  "Blueprint"
        3. Connect the repo:  broomva/parallax
        4. Render reads render.yaml from branch $BRANCH and shows the plan
        5. Name the blueprint, then "Apply" / "Create Resources"

      Every later edit to render.yaml is picked up on the next sync, so the
      file in this repo keeps describing the running service.
