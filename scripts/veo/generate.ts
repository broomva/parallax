 ...
 *
 * Raw Veo output lands in out/veo-raw/ (gitignored) and is re-encoded into
 * web/public/scroll-cinema/. The raw file is kept so a re-encode never costs
 * another generation.
 */
...
const BOARD = join(ROOT, "web", "scroll-cinema", "storyboard.json");
const RAW = join(ROOT, "out", "veo-raw");
const OUT = join(ROOT, "web", "public", "scroll-cinema");
...
const manifest = {
  generatedFrom: "web/scroll-cinema/storyboard.json",
  model: board.model,
