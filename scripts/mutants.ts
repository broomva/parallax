 ...
 * report lies:
 *
 *   1. It refuses to run against a dirty tree. Reverting is `git checkout --`,
 *      which would silently destroy uncommitted work, and every mutant after
 *      the first would then be patching an already-reverted file.
...
function gitStatus(): string {
  const p = Bun.spawnSync(["git", "-c", "core.fsmonitor=false", "status", "--porcelain"], {
    cwd: ROOT,
    env: process.env,
    stdout: "pipe",
    stderr: "pipe",
