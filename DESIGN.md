...
`web/app/globals.css`; what was missing was the statement, not the system. The header comment on that
...
Self-hosting a display face is deliberately **not** done. A hackathon judged surface that blocks on a
font CDN is a worse outcome than a system stack, and the system grotesk is not the tell — Inter
shipped as a decision nobody made is.

## Icons — decided

**There is no icon library, and adding one is a decision that needs a reason.** The repo depends on
...
- The **landing** pins light (`data-theme="light"`). Its two dark regions — the opening film and the
  ...
  follows the OS.
- **`/demo`** is dark end to end. That is not a mood: it is shown on a projector and restreamed, and
  the organisers' guidance is that a light deck washes out. It redefines the same roles under
  `--d-*` and scopes `color-scheme: dark` with `:has(.demo)` so the landing is untouched.
...
Spanish on `/demo` (it is the pitch, and the audience is in Bogotá); English on the landing and the
README (the audience is whoever reads the source).
