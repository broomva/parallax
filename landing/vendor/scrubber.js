 ...
 */
import { bindTargets, clamp01, easeToward, fadeAt, placement, residentSet, shouldReveal, shouldSeek, slotFor, timeFor, trackProgress, } from "./map.js";
const DEFAULTS = {
    tau: 0.096,
    fade: 0.1,
    maxResident: 3,
...
export function createScrollCinema(options) {
    const { clips, posters, stage, track, onScene, view = window, tau = DEFAULTS.tau, fade = DEFAULTS.fade, maxResident = DEFAULTS.maxResident, seekEpsilon = DEFAULTS.seekEpsilon, strategy = DEFAULTS.strategy, } = options;
    if (posters.length !== clips.length + 1) {
        throw new Error(`scroll-cinema: expected ${clips.length + 1} posters for ${clips.length} clips, got ${posters.length}`);
    }
     ...
     */
    const framesPresented = [0, 0];
    /** mediaTime of the most recent compositor-painted frame, per slot. */
    const lastPaintedTime = [Number.NaN, Number.NaN];
    /**
     ...
     * intended behaviour.
     */
    const revealLag = [Number.NaN, Number.NaN];
    ...
    if (retain !== maxResident) {
        console.warn(`scroll-cinema: maxResident ${maxResident} is below the floor of ${RETAIN_FLOOR} ` +
    ...
    function trackProgressNow() {
        if (track) {
            ...
            return trackProgress(-track.getBoundingClientRect().top, track.offsetHeight, view.innerHeight);
        }
    ...
    function urlFor(segment) {
        ...
        if (priorFailure) {
            if (priorFailure.attempts >= MAX_ATTEMPTS)
                return Promise.resolve(null);
            if (Date.now() < priorFailure.nextAt)
                return Promise.resolve(null);
        }
     ...
     */
    async function bind(segment) {
        if (destroyed || reducedMotion)
            return;
        if (segment < 0 || segment >= clips.length)
            return;
        const slot = slotFor(segment);
        if (held[slot] === segment)
            return;
        ...
        if (held[slot] === segment)
            return;
        if (!bindTargets(placement(easedProgress, clips.length).segment, clips.length).includes(segment)) {
            return;
        }
        held[slot] = segment;
    ...
    function release(targets) {
        for (let slot = 0; slot < videos.length; slot++) {
            const seg = held[slot];
            if (seg === null || targets.includes(seg))
                continue;
            held[slot] = null;
            presented[slot] = false;
            framesPresented[slot] = 0;
    ...
    function evict(keep) {
        ...
        for (const [k, url] of cache) {
            ...
            if (keep.includes(segment))
                continue;
            if (held[0] === segment || held[1] === segment)
                continue;
            if (strategy !== "stream") {
    ...
    // --- painting -----------------------------------------------------------
    const isReady = (v) => Number.isFinite(v.duration) && v.duration > 0 && v.readyState >= 2;
    function paint(segment, local, opacity) {
        if (reducedMotion)
            return;
        if (segment < 0 || segment >= clips.length)
            return;
        ...
        if (!revealed[slot] &&
            rvfcSupported &&
            !shouldReveal(lastPaintedTime[slot], t, REVEAL_TOLERANCE, performance.now() - (bindAt[slot] || 0), revealTimeoutMs)) {
            v.style.opacity = "0";
            return;
        }
